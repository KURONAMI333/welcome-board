...
public final class WelcomeBoardPreviewCommand {
    ...
    private static int preview(CommandSourceStack source) {
        ...
        // Deferred via schedule(), not execute(): the ChatScreen that dispatched this command calls
        // Gui#setScreen(null) right after handleChatInput() returns (see ChatScreen#keyPressed), on
        // the same call stack as this method — reached from GLFW input polling, not from inside a
        // queued task. BlockableEventLoop#execute() only queues when scheduleExecutables() is true
        // (not the client thread, or a task is already running); neither holds here, so execute()
        // would run this inline and ChatScreen's setScreen(null) would still win. schedule() always
        // enqueues onto pendingRunnables regardless, so it runs on the next runAllTasks() pass
        // (start of the next runTick()) — strictly after ChatScreen closes.
        Minecraft minecraft = Minecraft.getInstance();
        minecraft.schedule(() -> minecraft.gui.setScreen(new WelcomeBoardScreen(content, null)));
